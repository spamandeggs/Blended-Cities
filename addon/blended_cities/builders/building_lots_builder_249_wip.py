...
pdeb = True
minSideSize = 2        ## bMinSize
minBuildingArea = 10   ## bAreaInt[0]
minBuildingSize =  5.0 ## bSizeInt[0]
maxBuildingSize = 15.0 ## bSizeInt[1]
minIntervalSize =  0.0 ## iSizeInt[0]
maxIntervalSize =  1.0 ## iSizeInt[1]
IntervalSize = [minIntervalSize,maxIntervalSize]
BuildingSize = [minBuildingSize,maxBuildingSize]
stickedBuilding	=  0.5 ## bSticked   sticked building chances (1=sticked)
minl = 0.1
...
if A >= minBuildingArea :
	...
	#init - building starting and end points on the perimeter, reorder
	...
	while len(aPerim) > 0 :
		...
		if len(bobnewside) > 0 :
			...
			nap=[]
			for seg in aPerim :
				seg=polyBool2(seg,bobnewside,iSizeInt,bSticked,minl,pdeb=pdeb)
				nap.extend(seg)
			aPerim=[]
		...
		if len(aPerim) > 0 :
			...
			lenght0,dir = readVec(Vector(side[1])-Vector(side[0]))
			if len(side) == 3 :
			# last point of the last (angle) building
				lenghts = cutB(lenght0,[IntervalSize,BuildingSize],stickedBuilding)
				lenght,dirn=readVec(Vector(side[2])-Vector(side[1]))
				# enough lenght
				if lenght>=minBuildingSize + minIntervalSize:
				...
				if bldgs[bldgi]!=0 :
					...
					if len(bldg)==3 :
						if cfloat(bldg[0],'eq',bldg[1]) :
							l,d=readVec(Vector(bldg[2])-Vector(bldg[1]))
							coord=writeVec(0.1,d)
							bldg=[Vector(bldg[1])+coord,bldg[2]]
							if pdeb : print( 'case point too near : 3 -> 2 points',bldg)
						elif cfloat(bldg[1],'eq',bldg[2]) :
							l,d=readVec(Vector(bldg[0])-Vector(bldg[1]))
							coord=writeVec(0.1,d)
							bldg=[bldg[0],Vector(bldg[1])+coord]
							if pdeb : print('case point too near : 3 -> 2 points',bldg)
					...
					if pdeb : print('---')
					dprint('        .building %s (%s/%s) at %.2f,%.2f<DOT>',(len(buildingAreas)+bldgi+1,bldgi+1,len(bldgs),b1[0],b1[1]),level=3)
					#
					...
					if l>bsSizeInt[0] :
						# if l*lotBalance <= bsSizeInt[0] : l=l*lotBalance#Puniform(l*lotBalance,bsSizeInt[0])
						# else : l=Puniform(bsSizeInt[0],min(l*lotBalance,bsSizeInt[1]))
						if l <= bsSizeInt[1] and Puniform(0.0001,1)<=lotBalance :
							l=bsSizeInt[1]+1
							dprint('         max (l<max) : %s',l,level=3)
						elif bsSizeInt[0]+bsSizeInt[1]>l :
							l=Puniform(bsSizeInt[0],l-bsSizeInt[0])
							dprint('         reserve space : %s',l,level=3)
						else :
							l=Puniform(bsSizeInt[0],bsSizeInt[1])
...
else : buildingAreas.append([bob,pli])